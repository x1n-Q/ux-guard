...
✅ **`@x1n-q/ux-guard-mcp`** — full MCP server, verified working with the official MCP client SDK
...
**Before tuning:** ux-guard flagged 5 false-positive `missing_success_feedback` issues in `404game/src/games/*.ts` (Breakout, Dino, Flappy, Invaders, Snake) — because it saw `createBreakout()`, `update()`, etc. as "action handlers".
...
## 3. `@x1n-q/ux-guard-mcp` — the AI-agent moat 🤖

New 4th package: a full **Model Context Protocol** server that lets Claude Desktop / Cursor / Blackbox Code / any MCP client call ux-guard as a tool.

**Files:**
- `packages/mcp/src/index.ts` — stdio server, 4 tools
- `packages/mcp/test-smoke.js` — end-to-end smoke test using real MCP client SDK
- `packages/mcp/package.json` — exposes `ux-guard-mcp` bin
...
| `ux-guard_report`     | Markdown report (great for the agent to paste in a PR comment)|
| `ux-guard_list_rules` | All 6 rules with default severities                           |

**Verified working** — smoke test connects via real `StdioClientTransport`, lists tools, calls every tool, and validates outputs:

```
✓ connected to ux-guard MCP server
✓ listed 4 tools: ux-guard_scan, ux-guard_scan_json, ux-guard_report, ux-guard_list_rules
✓ ux-guard_scan returned summary (first 200 chars):
  ux-guard score: 0/100   (5 files, 11 issues: 4 error, 7 warn, 0 info)
  ...
✓ ux-guard_scan_json returned valid agent JSON (score=0, issues=11)
✓ ux-guard_list_rules returned 6 rules
✓ ux-guard_report returned markdown
...
{
  "mcpServers": {
    "ux-guard": { "command": "npx", "args": ["-y", "@x1n-q/ux-guard-mcp"] }
  }
}
```

Now an agent can run ux-guard **before** saying "feature done." That's the whole selling point of the product — and it works.
...
- `package.json` (root) — added vitest, build now includes `@x1n-q/ux-guard-mcp`
...
1. **`npm link` in `packages/cli/`** so you can run `ux-guard scan ./src` from anywhere on this machine and try it on more real projects.
2. **Hook up the MCP server to your local agent** (Claude Desktop / Cursor) and feel what it's like to have an AI run ux-guard automatically.
3. **Init a git repo** and push to GitHub — `cd /data/data/com.termux/files/home/projects/ux-guard && git init && git add . && git commit -m "feat: ux-guard MVP with MCP + tests"`.
4. **Publish** — check if `ux-guard` is taken on npm; if not, `npm publish --access public` from each package (core, react, cli, mcp). Versioned at `0.1.0`.
