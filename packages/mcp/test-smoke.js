// Smoke test for the ux-guard MCP server.
// Spawns the server as a child process and uses the MCP client SDK
// to: list tools, then call ux-guard_scan on the example sample app.
...
async function main() {
  ...
  const client = new Client(
    { name: "ux-guard-smoke-test", version: "0.0.0" },
    { capabilities: {} },
  );

  await client.connect(transport);
  console.log("✓ connected to ux-guard MCP server");
  ...
  console.log("✓ ux-guard_scan returned summary (first 200 chars):");
  console.log("  " + scanText.slice(0, 200).replace(/\n/g, "\n  "));
  if (!/score:\s*\d+\/100/.test(scanText)) {
    throw new Error("ux-guard_scan output missing score");
  }
  ...
  if (parsed.task !== "ux_completeness_scan") {
    throw new Error("ux-guard_scan_json missing task field");
  }
  console.log(
    `✓ ux-guard_scan_json returned valid agent JSON (score=${parsed.score}, issues=${parsed.issues.length})`,
  );
  ...
  if (!/missing_loading_state/.test(rulesText)) {
    throw new Error("ux-guard_list_rules missing expected rule");
  }
  console.log("✓ ux-guard_list_rules returned 6 rules");
  ...
  if (!/## UX Completeness Report/.test(reportText)) {
    throw new Error("ux-guard_report output missing markdown header");
  }
  console.log("✓ ux-guard_report returned markdown");
