...
uxlint understands how the **Next.js App Router** handles UX states at the
route level and won't double-flag pages that are already covered.

## How it works

For every file uxlint scans, it walks **up** from the file's directory until
...
- **Pages Router** projects (`pages/`) — uxlint treats them like normal
...
uxlint caches the route lookup per-directory during a single scan, so
